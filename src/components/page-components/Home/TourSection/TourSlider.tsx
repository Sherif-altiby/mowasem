...
export default function TourSlider({ data: tours = [] }: { data: Daum[] }) {
  ...
  useLayoutEffect(() => {
    ...
    if (!track || !viewport || displayTours.length === 0) return;
    ...
    return () => ctx.revert();
  }, [displayTours.length, updateNavState]);
  ...
  return (
    <div className="relative w-full group">
      ...
      {/* Nav arrows: in RTL, "next" reveals cards toward the left, so it sits on the LEFT;
         "prev" goes back toward the start, so it sits on the RIGHT. */}
      <button
        type="button"
        ...
        className="
          ...
          cursor-pointer
          disabled:bg-white
          disabled:border-[#676768]
          disabled:text-[#676768]
